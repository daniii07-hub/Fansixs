...
export async function movePlannerEvent(
  ...
  try {
    ...
    if (input.startTime && startTime === null) {
      return {
        success: false,
...
export async function updatePlannerEventTechnician({
  workOrderId,
  technician,
}: {
  workOrderId: number;
  ...
  try {
    ...
    if (error) {
      console.error(
        "Update planner technician error:",
        error,
      );
    ...
    return {
      success: true,
      message: normalizedTechnician
        ? `Jobbet har tilldelats ${normalizedTechnician}.`
        : "Teknikern har tagits bort från jobbet.",
    };
  } catch (error) {
    console.error(
      "Update planner event technician error:",
      error,
    );
